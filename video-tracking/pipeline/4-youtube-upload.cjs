 ...
 *   2. Create project "Discover Puerto Rico"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Puerto Rico Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'puerto-rico'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Puerto Rico ${year}`,
      description: `${title}\n\nFull guide: https://discoverpuertorico.info/${slug}/`,
      tags: [`Vietnam ${slug}`, 'Puerto Rico travel', `Vietnam ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Puerto Rico ✈️ #shorts`,
      description: `${title} — Full guide: https://discoverpuertorico.info/destinations/${slug}/`,
      tags: [`${title} Puerto Rico`, 'Puerto Rico travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Puerto Rico — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Puerto Rico for years with deep local knowledge. No sponsored content.

📖 FULL WRITTEN GUIDE: https://discoverpuertorico.info/destinations/${slug}/
📱 TRIP COMPANION (works offline): https://discoverpuertorico.info/companion/

#PuertoRico #${title.replace(/\s+/g, '')} #PuertoRicoTravel #TravelGuide${year}`;
}
...
function generateDestTags(slug, title, dest) {
  const tags = [
    `${title} Puerto Rico`,
    `${title} travel guide`,
    `Puerto Rico ${new Date().getFullYear()}`,
    `${title} things to do`,
    `where to stay ${title}`,
    'Puerto Rico travel',
    'Puerto Rico food',
    'Puerto Rico budget travel',
    'Puerto Rico tips',
  ];
  if (dest?.region) tags.push(`${dest.region} Puerto Rico`);
