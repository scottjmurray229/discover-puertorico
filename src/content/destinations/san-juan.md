---
title: "San Juan"
description: "Explore Old San Juan's colorful colonial streets, historic forts, vibrant nightlife, and world-class dining in Puerto Rico's capital city."
heroImage: ""
heroAlt: "Condado Beach skyline and Atlantic surf, San Juan, Puerto Rico"
tagline: "500 years of Spanish colonial history inside walls that still stand — and the best mofongo, rum, and nightlife in the Caribbean"
region: "metro"
bestMonths: ["Dec", "Jan", "Feb", "Mar", "Apr"]
budgetPerDay:
  backpacker: 80
  midRange: 200
  luxury: 500
gettingThere: "Fly into Luis Muñoz Marín International Airport (SJU), the main hub for all of Puerto Rico. Uber and Lyft operate island-wide. The airport is 15 minutes from Condado and 20 minutes from Old San Juan. No passport needed for US citizens."
essentials:
  - { icon: "🌡️", label: "Climate", value: "Tropical — warm year-round, 80-90°F" }
  - { icon: "💵", label: "Budget", value: "$80-500/day" }
  - { icon: "🗣️", label: "Language", value: "Spanish & English (bilingual)" }
  - { icon: "🕐", label: "Time Zone", value: "AST (UTC-4, no daylight saving)" }
highlights:
  - "Walk the blue cobblestone streets of Old San Juan past 500-year-old pastel colonial buildings"
  - "Explore Castillo San Felipe del Morro, the iconic 16th-century fortress guarding the harbor"
  - "Bar-hop through La Placita de Santurce on a Thursday night with locals"
  - "Watch the sunset from the ancient city walls with a piña colada in hand"
gradientColors: "from-blue-900 via-teal-800 to-blue-900"
relatedDestinations:
  - "condado"
  - "isla-verde"
  - "el-yunque"
faqItems:
  - { question: "Is San Juan safe for tourists?", answer: "Old San Juan, Condado, and Isla Verde are very safe tourist areas with regular police presence. Use the same common sense you'd use in any US city — don't flash valuables, stick to well-lit streets at night, and use rideshare apps. La Perla neighborhood has improved significantly but visit during daytime." }
  - { question: "How many days do I need in San Juan?", answer: "Three full days covers Old San Juan's forts and streets, Santurce's art and food scene, and a beach day. Add a fourth day for a day trip to El Yunque rainforest or a food tour through Piñones. A week lets you really settle in and explore neighborhoods like Ocean Park and Miramar." }
  - { question: "Do I need to speak Spanish in San Juan?", answer: "No. San Juan is fully bilingual and most people in tourist areas speak English fluently. That said, even basic Spanish effort is appreciated and will earn you smiles. Puerto Rico is a US territory so your US phone plan works here too." }
  - { question: "What is the best neighborhood to stay in San Juan?", answer: "Old San Juan for atmosphere and walkability — you are steps from everything and experience the city at its most beautiful. Condado for beach access, upscale dining, and a more resort-style stay. Isla Verde for the widest beach and the most hotel options. Ocean Park is a quiet residential neighborhood with good guesthouses between Condado and Isla Verde — often the best value." }
  - { question: "What is mofongo and where should I try it?", answer: "Mofongo is Puerto Rico's signature dish — mashed green plantain formed into a bowl and stuffed with your choice of filling (garlic shrimp is traditional, but crab, chicken, and lobster versions exist). Essential at least once during any visit. In Old San Juan, El Jibarito on Calle Sol is the most famous and worth the line. La Factoria nearby is excellent and less crowded." }
  - { question: "Do I need a car in San Juan?", answer: "Not if you are staying in Old San Juan, Condado, or Isla Verde — these three neighborhoods are walkable and well-served by Uber and Lyft. A car is essential if you want to explore beyond San Juan: El Yunque, Fajardo, the eastern bioluminescent bay, Ponce, or Rincón all require wheels. Rental cars are available at the airport and around the city." }
  - { question: "What is the best time of year to visit San Juan?", answer: "December through April is peak season and best weather — drier, cooler, and pleasantly breezy. January through March is the sweet spot for combining Old San Juan sightseeing with beach time. Summer (June-August) is hot and occasionally rainy but still very popular, and accommodation prices drop. Hurricane season runs June through November with September and October being the highest-risk months." }
  - { question: "How do I get from San Juan airport to Old San Juan?", answer: "Uber or Lyft from Luis Muñoz Marín Airport (SJU) to Old San Juan takes 20-25 minutes and costs $20-30 USD. A taxi costs slightly more ($30-40 fixed rate). The public AMA bus (T5 route) is under $2 but takes 45-60 minutes. There is no direct metro or train connection. Most visitors opt for rideshare for the ease." }
affiliatePicks:
  - name: "Hotel El Convento"
    type: "hotel"
    price: "$250/night"
    personalNote: "A converted 17th-century convent in the heart of Old San Juan. The rooftop pool overlooks the cathedral and the harbor. Worth every penny for the location alone."
    affiliateUrl: "#"
  - name: "Fortaleza Guest House"
    type: "hotel"
    price: "$95/night"
    personalNote: "Best budget option in Old San Juan. Clean rooms, great location on Calle Fortaleza, and a rooftop terrace. Book early — it fills up fast."
    affiliateUrl: "#"
  - name: "Old San Juan Walking Food Tour"
    type: "tour"
    price: "$75/person"
    personalNote: "Five stops over three hours covering mofongo, alcapurrias, local coffee, and rum. The guide knows every hidden courtyard and back alley. Don't eat breakfast before."
    affiliateUrl: "#"
lastVerified: 2026-02-28
contentStatus: "published"
draft: false
---

## Why San Juan Belongs on Your List

San Juan isn't just Puerto Rico's capital — it's one of the oldest European-founded cities in the Americas, and it wears those 500 years beautifully. The old city sits on a narrow islet connected to the main island, its blue cobblestone streets lined with pastel colonial buildings that have been meticulously restored without losing their soul.

But San Juan is more than a museum. This is a city that stays up late, eats well, and knows how to throw a party. Thursday nights at La Placita de Santurce transform a quiet farmers' market into one of the best outdoor bar scenes in the Caribbean. The Santurce art district rivals anything in Miami. And the food — from roadside alcapurrias to Michelin-quality restaurants — punches far above its weight.

<immersive-break-inline videoSrc="/videos/destinations/san-juan-hero.mp4" heading="500 Years of History Meet Caribbean Soul" subheading="Blue cobblestones, colonial fortresses, and a food scene that punches far above its weight." />

## Things to Do

**Castillo San Felipe del Morro** — The crown jewel. This massive 16th-century fortress took 250 years to build and guards the entrance to San Juan Bay. Entry is $10 and includes both El Morro and Castillo San Cristóbal. Go early morning or late afternoon for the best light and fewer crowds.

**Walk Calle Fortaleza & Calle San Sebastián** — The two main arteries of Old San Juan. Fortaleza has the shops and restaurants; San Sebastián has the bars and galleries. Both are gorgeous at any hour.

**La Placita de Santurce** — On Thursday through Saturday nights, this daytime farmers' market becomes the island's best nightlife. Dozens of bars spill onto the streets, salsa music fills the air, and it's 90% locals. Start around 9 PM.

**Paseo de la Princesa** — A tree-lined promenade along the old city walls that ends at a beautiful fountain overlooking the bay. Best at sunset. Street vendors sell coconut ice cream and piraguas (shaved ice).

**Museo de Arte de Puerto Rico** — World-class art museum in Santurce with both Puerto Rican and international collections. The sculpture garden alone is worth the visit. $6 admission.

## Where to Stay

**Hotel El Convento** — $250/night. Historic luxury in a converted convent. Rooftop pool, perfect location. The standard for Old San Juan stays.

**The Gallery Inn** — $180/night. Eccentric, art-filled guesthouse at the highest point of Old San Juan. Every room is different. The rooftop terrace has the best views in the city.

**Fortaleza Guest House** — $95/night. Clean, affordable rooms on the main strip. No frills but unbeatable location.

**Condado Vanderbilt** — $350/night. Beachfront luxury in Condado. Full-service spa, oceanfront pool, and three excellent restaurants.

## Where to Eat

**Marmalade** — Fine dining on Calle Fortaleza. The tasting menu ($85) is one of the best meals on the island. Reservations essential.

**La Casita Blanca** — Hole-in-the-wall in Santurce serving the best traditional Puerto Rican food in the city. The mofongo relleno de camarones is legendary. $12-18 per plate.

**Pirilo Pizza Rústica** — Late-night pizza spot in Old San Juan with craft cocktails. Perfect post-bar fuel. $15-20.

**Café Don Ruiz** — Best local coffee in Old San Juan. Puerto Rican single-origin beans roasted on-site. A cortadito here costs $3 and is better than anything in Miami.

**Santaella** — Chef José Santaella's flagship in Santurce. Modern Puerto Rican cuisine in a gorgeous space. The whole fried snapper ($32) is a showstopper.

## Scott's Pro Tips

<div class="scott-tips">

**Getting Around** — Old San Juan is entirely walkable but hilly. Wear comfortable shoes. For getting between Old San Juan, Condado, and Santurce, use Uber or Lyft ($5-10 between neighborhoods). The free trolley runs through Old San Juan during the day.

**Best Time to Visit** — December through April is peak season with perfect weather (low 80s, minimal rain). June through November is hurricane season but also means lower prices and fewer crowds. September and October are the highest-risk months.

**Money** — Puerto Rico uses US dollars. Your US bank cards, credit cards, and phone plan all work normally. No currency exchange needed.

**Safety** — Old San Juan, Condado, Isla Verde, and Ocean Park are very safe. Santurce is fine during the day and at La Placita at night. Use rideshare apps rather than walking alone through unfamiliar neighborhoods late at night.

**Packing** — Sunscreen (the tropical sun is intense), comfortable walking shoes for cobblestones, a light rain jacket, and something decent for dinner out. Old San Juan restaurants range from casual to dressy.

**Local Culture** — Puerto Ricans are incredibly warm and welcoming. "¡Wepa!" is the universal expression of excitement. Tipping is standard (18-20% at restaurants, $1-2 per drink). Most people are bilingual but appreciate any Spanish effort.

</div>
