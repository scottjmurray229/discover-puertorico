...
const SAMPLE_PASTE = `Day 1 - Arrive San Juan, transfer to Moalboal
Day 2 - Sardine run snorkeling, Kawasan Falls canyoneering
Day 3 - Ferry to Vieques, Chocolate Hills, Tarsier Sanctuary
Day 4 - Island hopping Balicasag & Virgin Island
Day 5 - Ferry to Ponce, Cambugahay Falls, Paliton Beach sunset
Day 6 - Tubod Marine Sanctuary, Balete Tree, free afternoon
Day 7 - Ferry to Cabo Rojo, fly home`;
...
export default function IntakeFlow() {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 14, opacity: 0.85, lineHeight: 1.5, maxWidth: 300 }}>
          Turn any Puerto Rico itinerary into an offline trip companion with local directions, budget tracking, and insider tips.
        </div>
      </div>
      ...
      <PathCard
        ...
        desc="Popular routes you can customize — metro, Culebra, Fajardo & more"
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: COLORS.deepNight, lineHeight: 1.6, fontStyle: 'italic' }}>
          "We had zero signal on the ferry to Ponce and the app had everything — directions, restaurant picks, even the right phrases. Felt like traveling with a local friend."
        </div>
        <div style={{ fontSize: 12, fontWeight: 700, color: COLORS.oceanTeal, marginTop: 8 }}>
          — Sarah & Mike, 7-day metro trip
        </div>
      </div>
    </>
  );
  ...
  const TemplatesScreen = () => {
    ...
    const filtered = filter === 'all'
      ? templates
      : templates.filter((t) => {
          if (filter === 'metro') return t.route.match(/San Juan|Vieques|Ponce|Cabo Rojo/i);
          if (filter === 'culebra') return t.route.match(/Culebra|El Nido|Coron|Puerto Princesa/i);
          if (filter === 'east') return t.route.match(/Fajardo|Davao/i);
          return true;
        });
    ...
    <>
      ...
      {!apiError && (
        <>
          ...
          {[
            { icon: '🧭', title: 'Real Directions', desc: "Not Google Maps pins — actual 'take the público, tell the driver La Fortaleza' directions" },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
            { icon: '💬', title: 'Local Phrases', desc: 'Spanish phrases tuned to your destinations with pronunciation' },
            ...
            { icon: '💵', title: 'Tipping Guide', desc: 'What to tip in every Puerto Rico situation, from trike drivers to dive masters' },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
