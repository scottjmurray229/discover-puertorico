...
function splitDescription(description: string, priceUsd?: number): { title: string; detail: string } {
  ...
  if (priceUsd) {
    detail += ` ($${priceUsd.toLocaleString()})`;
  }
...
export function transformItinerary(apiItinerary: ApiItinerary): TripData {
  ...
  const contacts: Contact[] = [
    { icon: '🚨', name: 'Puerto Rican Emergency', role: 'Emergency Services', phone: '911' },
    { icon: '🏥', name: 'Puerto Rican Red Cross', role: 'Nationwide', phone: '143' },
  ];
