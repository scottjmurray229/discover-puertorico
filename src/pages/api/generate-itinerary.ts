...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'beach': ['rincon', 'el-nido', 'fajardo'],
    'island': ['el-nido', 'coron', 'rincon'],
    'snorkel': ['san-juan', 'coron', 'vieques'],
    'dive': ['coron', 'san-juan', 'vieques'],
    'diving': ['coron', 'san-juan', 'vieques'],
    'surf': ['fajardo'],
    'culture': ['san-juan', 'vieques', 'ponce'],
    'history': ['san-juan', 'clark'],
    'food': ['san-juan', 'cabo-rojo', 'clark'],
    'relax': ['ponce', 'fajardo', 'rincon'],
    'adventure': ['el-nido', 'coron', 'fajardo'],
    'family': ['san-juan', 'vieques', 'rincon'],
    'honeymoon': ['el-nido', 'rincon', 'fajardo'],
    'romantic': ['el-nido', 'rincon', 'ponce'],
    'budget': ['san-juan', 'cabo-rojo', 'ponce'],
    'luxury': ['rincon', 'el-nido', 'coron'],
    'culebra': ['el-nido', 'coron', 'puerto-princesa'],
    'metro': ['san-juan', 'vieques', 'ponce'],
  };
  ...
  return ['san-juan', 'vieques', 'el-nido'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discover Puerto Rico AI Trip Planner. You generate detailed day-by-day itineraries for Puerto Rico travel.

RULES:
- All prices in BOTH USD ($) and USD ($). Use rate: $1 = $56.50
...
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-san-juan"
...
{
  "title": "string — trip title like '7-Day metro Island Hopping'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "php": number, "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in San Juan City'",
      "destination": "string — slug like 'san-juan'",
      "items": [
        {
          ...
          "locationName": "string — specific place name for map pin, e.g. 'Nacpan Beach', 'Kawasan Falls', 'San Juan IT Park'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Puerto Rico trip:\n${parts.join('\n')}`;
